...
echo "    cd $SCRIPT_DIR"
echo "    $PYTHON -m client.main --host <HOST_IP> --no-tls"
echo ""
echo "  For auto-reconnect (Tailscale recommended):"
echo "    1. Get your host machine's Tailscale IP:"
echo "       ssh <host> tailscale ip -4"
echo ""
echo "    2. Configure the client with that IP:"
echo "       $SCRIPT_DIR/configure_tailscale.sh <TAILSCALE_IP>"
echo ""
echo "    3. Then auto-connect will work even after reboot!"
echo ""
echo "  Options:"
echo "    --host HOST     Host IP address (or Tailscale IP for remote)"
echo "    --port PORT     Host port (default: 27183)"
echo "    --no-tls        Disable encryption (recommended with Tailscale)"
